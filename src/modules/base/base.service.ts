...
export class BaseService<T = BaseDocument> {
  ...
  async create(data: RequireAtLeastOne<Omit<T, 'id' | 'createdAt' | 'updatedAt'>>): Promise<T> {
  ...
  async findOneOrCreate(data: RequireAtLeastOne<T>, payload?: RequireAtLeastOne<T>): Promise<T> {
    const result = await this.model.findOne(data as T)
    return result ? Promise.resolve(result) : new this.model({ ...payload, ...data }).save()
  }

  async updateOneById(id: BaseDocument['id'], data: Partial<T>): Promise<T> {
    return await this.model.findOneAndUpdate({ id }, data as T, { new: true }).then((result: T) => {
      if (!result) throw new NotFoundException()
      return result
    })
  }
